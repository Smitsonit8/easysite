...
?>
<section class="sporina-vacancies-list">
<?php foreach ($sections as $section): if (empty($section['ITEMS'])) continue; ?>
    <h3 class="sporina-vacancies-list__section-title"><?=htmlspecialcharsbx($section['NAME'])?></h3>
    <div class="sporina-vacancies-list__section">
    <?php foreach ($section['ITEMS'] as $item):
        $this->AddEditAction($item['ID'], $item['EDIT_LINK'], CIBlock::GetArrayByID($item['IBLOCK_ID'], 'ELEMENT_EDIT'));
        $this->AddDeleteAction($item['ID'], $item['DELETE_LINK'], CIBlock::GetArrayByID($item['IBLOCK_ID'], 'ELEMENT_DELETE'), array('CONFIRM' => GetMessage('CT_BNL_ELEMENT_DELETE_CONFIRM')));
        $get = function($name) use ($item, $arParams) { $code = isset($arParams['PROPERTY_'.$name]) ? $arParams['PROPERTY_'.$name] : ''; return $code !== '' && isset($item['PROPERTIES'][$code]['VALUE']) ? $item['PROPERTIES'][$code]['VALUE'] : ''; };
        $meta = array_filter(array($get('CITY'), $get('EXP'), $get('TYPE')));

    ?>
        <a class="sporina-vacancies-list__item" href="<?=$item['DETAIL_PAGE_URL']?>" id="<?=$this->GetEditAreaId($item['ID'])?>">
            <span class="sporina-vacancies-list__main"><span class="sporina-vacancies-list__name"><?=$item['NAME']?></span>
                <?php if (!empty($meta)): ?>
                    <span class="sporina-vacancies-list__meta"><?=implode(' / ', $meta)?></span>
                <?php endif; ?>
            </span>
            <?php if ($get('SALARY') !== ''): ?>
                <span class="sporina-vacancies-list__salary"><?=$get('WAGE')?></span>
            <?php endif; ?>
            <span class="sporina-vacancies-list__arrow">
