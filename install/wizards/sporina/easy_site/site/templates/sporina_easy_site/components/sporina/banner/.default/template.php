		...
		<div class="sporina-banner__content">
			...
			<?php if ($arResult["SHOW_BUTTON"]): ?>
				<?php if ($arResult["BUTTON_ACTION"] === "form"): ?>
					<button class="sporina-button sporina-banner__button" type="button" data-banner-form-open="<?=$formModalId?>" aria-controls="<?=$formModalId?>" aria-expanded="false" aria-haspopup="dialog"><?=htmlspecialcharsbx($arResult["BUTTON_TEXT"])?></button>
				<?php else: ?>
					<a class="sporina-button sporina-banner__button" href="<?=htmlspecialcharsbx($arResult["BUTTON_LINK"])?>"><?=htmlspecialcharsbx($arResult["BUTTON_TEXT"])?></a>
				<?php endif; ?>
			<?php endif; ?>
		</div>
