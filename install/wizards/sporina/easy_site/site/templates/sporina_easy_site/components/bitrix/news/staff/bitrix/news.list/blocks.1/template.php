...
$propertyValue = function ($item, $parameter) use ($arParams) {
    $code = isset($arParams[$parameter]) ? $arParams[$parameter] : '';
    return $code !== '' && isset($item['DISPLAY_PROPERTIES'][$code]['DISPLAY_VALUE']) ? $item['DISPLAY_PROPERTIES'][$code]['DISPLAY_VALUE'] : '';
};
...
?>
<div class="sporina-staff sporina-staff--blocks">
    <?php if ($arParams['DISPLAY_TOP_PAGER'] === 'Y') echo $arResult['NAV_STRING']; ?>
    <div class="sporina-staff__grid">
        <?php foreach ($arResult['ITEMS'] as $item) {
            ...
            $picture = !empty($item['PREVIEW_PICTURE']) ? $item['PREVIEW_PICTURE'] : $item['DETAIL_PICTURE'];
            $picture = $picture ? CFile::ResizeImageGet($picture, array('width' => 550, 'height' => 550), BX_RESIZE_IMAGE_EXACT)['src'] : SITE_TEMPLATE_PATH.'/images/picture.missing.png';
            $phone = $propertyValue($item, 'PROPERTY_PHONE'); $email = $propertyValue($item, 'PROPERTY_EMAIL');
        ?>
        <article id="<?= $id ?>" class="sporina-staff__card">
