...
<div class="sporina-staff sporina-staff--list">
    ...
    <article id="<?= $id ?>" class="sporina-staff__row">
        ...
        <div class="sporina-staff__body">
            ...
            <a class="sporina-staff__name" href="<?= $item['DETAIL_PAGE_URL'] ?>"><?= $item['NAME'] ?></a>
            <?php if ($phone || $email) { ?><div class="sporina-staff__contacts"><?php if ($email) { ?><a href="mailto:<?= htmlspecialcharsbx(strip_tags($email)) ?>"><?= file_get_contents(__DIR__.'/svg/contact.email.svg') ?><span><?= $email ?></span></a><?php } ?><?php if ($phone) { ?><a href="tel:<?= htmlspecialcharsbx(preg_replace('/[^+0-9]/', '', strip_tags($phone))) ?>"><?= file_get_contents(__DIR__.'/svg/contact.phone.svg') ?><span><?= $phone ?></span></a><?php } ?></div><?php } ?>
