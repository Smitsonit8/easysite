...
<section class="sporina-news-timeline <?=htmlspecialcharsbx($layoutClass)?>">
	<div class="sporina-news-timeline__head">
		<?if (($arParams['SHOW_IBLOCK_TITLE'] ?? 'Y') === 'Y' && !empty($arResult['NAME'])):?>
			<h3 class="sporina-news-timeline__title"><?=htmlspecialcharsbx($arResult['NAME'])?></h3>
		<?endif;?>
	</div>
	...
	<?if(($arParams["SHOW_MORE_BUTTON"] ?? "Y") !== "N" && $listPageUrl !== ""):?>
		<div class="sporina-news-timeline__actions">
			<a class="sporina-news-timeline__all sporina-button" href="<?=$listPageUrl?>"><?=GetMessage("SPORINA_SHOW_MORE")?></a>
		</div>
	<?endif;?>
</section>
<?endif;?>
